...
| Layer | Choice |
|---|---|
| Database, auth, storage | **Supabase** — the same project as the PC Prime client portal |
| Front end | **Next.js (App Router) + TypeScript**, deployed on **Vercel** |
| Charts | Inline SVG, no charting library (the prototype's charts are self-contained and can be ported) |
| File parsing | Server-side in Supabase **Edge Functions** (Deno) or a Next.js route handler |

### Why one Supabase project and two front ends

The reporting application is a **separate deployment** (its own subdomain, e.g.
`reporting.pcprime…`) sharing the portal's **single** Supabase project.

- One database, because two databases means two client registers and eventually a
  mismatch. The client master lives once, in the portal's `public.clients`.
- Separate deployments, because the portal is client-facing and live, and the staff
  review tool changes weekly. A staff release must not be able to break the portal.
- Two RLS policy sets on the same tables: staff see every client they are granted;
  a client, later, sees only their own and only finished packs. The client-facing
  view then costs a policy, not a rebuild.
...
/supabase
  /migrations
    0001_core.sql          # supplied — schema, RLS, commit_ledger_import
    0002_reporting.sql     # supplied — review, VAT, payroll, stock, budgets, company
  /functions
    import-file/           # receives an upload, dispatches to a parser, stages, commits
    run-checks/            # re-runs the review engine after every commit
/app
  (auth)/sign-in           # client chosen HERE, then user and password
  [clientSlug]/
    overview/ management/ profit-loss/ balance-sheet/ cash-flow/ expenses/
    sales/ budget/
    statements/ account-movements/ transactions/ ledgers/ stock/ payroll/ vat/
    review/ audit/
    data-import/ account-mapping/ company/ client-setup/
/lib
  btms/                    # the parsers — see section 6
  reports/                 # report-line aggregation, comparatives, periods
  checks/                  # the review engine
/reference
...
`PCP client reporting - TEMPLATE.html` in the CLIENTS DATA folder is a **complete,
working, single-file implementation** with two real clients embedded, and it is the
...
Apply `0001_core.sql` then `0002_reporting.sql` (both supplied alongside this file).
...
`has_client_access(uuid)` is `security definer` and reads `client_access`. There is
no service-role query path in the application. A query that forgets its client
filter returns **zero rows**, not another client's ledger.

**Acceptance test, must be written first and must pass:** a user granted client A
who selects from `postings` with no filter gets only client A's rows; the same user
selecting `where client_id = <B>` gets zero rows, not an error and not data.
...
**P0 — Isolation.** Migrations applied, RLS live, the cross-client acceptance test
in section 5 passing. *Done when a granted user provably cannot read another
client's rows.*
