...
-- Migration 191: review, VAT, payroll, stock, budgets, company record
--
-- Depends on 190_reporting_core.sql. Same rule throughout: every table
-- carries client_id, RLS on, one policy that defers to the portal's own
-- public.user_can_access_client(bigint). No service-role path in the app.
-- =====================================================================
...
-- client access. The flag goes on the portal's own link table so there is
-- still only ONE register of who may see what.
...
create or replace function has_payroll_access(target bigint) returns boolean
language sql stable security definer set search_path = reporting, public as $$
  select reporting.staff_can_access(target) and (
    public.is_admin() or exists (
      select 1 from public.user_clients uc
      where uc.user_id = auth.uid() and uc.client_id = target and uc.payroll_access
    )
  );
$$;
...
create or replace function prune_signoffs(p_client bigint) returns integer
