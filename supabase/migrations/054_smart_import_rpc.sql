-- =============================================================
-- Migration 054: smart_import RPC (UI Polish v2 — Smart Import Phase 6)
-- Run this in Supabase Dashboard → SQL Editor → New Query
-- =============================================================
-- Applies a Smart Import run: creates new clients and merges data into
-- existing ones, plus fills empty client_addresses. The frontend has
-- already done the column mapping, validation and per-client review —
-- this RPC just applies the resolved decisions.
--
-- Per-row atomicity: each row runs in its own savepoint (BEGIN/EXCEPTION),
-- so one bad row never aborts the others. Successful rows commit; failures
-- are returned per-row. The import is safe to re-run (re-running matches
-- the already-imported clients and becomes updates / no-change).
--
-- p_rows is a JSON array of row operations:
--   { "action":"create", "fields":{...},
--     "addresses": { "registered": {line1,line2,city,postal_code,country}, ... } }
--   { "action":"update", "client_id":123, "fields":{...}, "addresses":{...} }
-- Dates in `fields` must be ISO (YYYY-MM-DD); `email` a JSON array of strings.
-- =============================================================
...
begin
  ...
  loop
    ...
    begin
      ...
      if v_action = 'create' then
        ...
        -- Existing row, with the incoming fields layered on top.
  ...
  -- One summary audit row (per-row inserts/updates are also captured by the
  -- normal clients audit trigger).
  ...
  values (
    ...
    jsonb_build_object('batch_id', v_batch, 'created', v_created,
