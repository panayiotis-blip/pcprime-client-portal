...
// ---- patch 28: expense analysis, two views and the detail (FIX-3 §8) ----
//
// §8a months across or years across, §8b two levels of drill-down, §8c sort
// by any column and hide the nil lines. renderExp is replaced whole again:
// patch 22 gave it the comparison columns, and this gives it everything else,
// including taking the sparkline out.
{
  const selWas = "  [\"sumYear\",\"budYear\",\"cmYear\",\"cioYear\"].forEach(id=>{const el=document.getElementById(id);if(el)fillYears(el)});";
  if (!script.includes(selWas)) throw new Error('the year pickers are not where they were.');
  script = script.replace(selWas, "  [\"sumYear\",\"budYear\",\"cmYear\",\"cioYear\",\"expYear\"].forEach(id=>{const el=document.getElementById(id);if(el)fillYears(el)});");

  const [a, b] = cutFn(script, 'renderExp');
  script = script.slice(0, a) + "/* Expense analysis (FIX-3 §8).\n *\n * Two ways to lay it out, chosen at the top: months across for one year, or\n * years across through the comparison list every other statement uses. The\n * MONTHLY SHAPE sparkline is gone — it was too small to read a figure off, and\n * real month columns are what it was standing in for.\n *\n * Every line here sums several nominal accounts, and until now there was no way\n * to see inside one. Pressing a line opens the accounts beneath it across the\n * same columns; pressing an account opens the postings, with the journal and the\n * reference, so the item can be found in BTMS. That is the answer to the\n * question this screen provokes and could not answer: what was in it last year\n * that is not in it this year.\n */\nvar EXPVIEW=\"years\";        /* or \"months\" */\nvar EXPOPEN={line:null,acct:null};\nvar EXPSORT={k:0,dir:-1};   /* which column, and which way */\nvar EXPNIL=false;           /* show lines that are nil everywhere */\n\n/* The columns, whichever view is on. A column is a range of months and nothing\n   else, so everything below can treat the two views the same way. */\nfunction expColumns(){\n  if(EXPVIEW===\"months\"){\n    const y=document.getElementById('expYear').value, out=[];\n    for(let k=1;k<=12;k++){const j=idx(y+\"-\"+String(k).padStart(2,\"0\"));\n      if(j>=0)out.push({label:MN[k-1],range:[j,j]});}\n    return out;\n  }\n  const r=periodRange(\"exp\");\n  return [{label:spanLbl(r),range:r}].concat(cmpCols(\"exp\").map(function(c){\n    return c.missing?{label:c.label,missing:c.missing}\n                    :(c.budget?{label:c.label,budget:true}:{label:c.label,range:c.range});\n  }));\n}\n/* What a monthly series is worth in a column. A budget column can answer for a\n   report line and not for a nominal account, which is said rather than guessed. */\nfunction expOver(arr,c){\n  if(c.missing||c.budget)return null;\n  return arr.slice(c.range[0],c.range[1]+1).reduce((a,b)=>a+b,0);\n}\n\nfunction renderExp(){\n  const months=EXPVIEW===\"months\";\n  periodCtl(\"exp\",\"expCtl\",renderExp);\n  cmpCtl(\"exp\",\"expCmpBar\",renderExp);\n  /* Only the controls the chosen view needs. Guarded, because a render must\n     not die on a screen that is missing a control it merely hides. */\n  const show=(id,on)=>{const el=document.getElementById(id); if(el&&el.style)el.style.display=on?\"\":\"none\";};\n  show('expCtl',!months); show('expCmpBar',!months); show('expYearBar',months);\n  const vb=document.getElementById('expView');\n  if(vb)vb.innerHTML=[[\"years\",\"Years across\"],[\"months\",\"Months across\"]]\n    .map(v=>`<button class=\"perbtn${EXPVIEW===v[0]?\" on\":\"\"}\" data-view=\"${v[0]}\">${v[1]}</button>`).join(\"\")\n    +`<button class=\"perbtn${EXPNIL?\" on\":\"\"}\" id=\"expNil\">${EXPNIL?\"Showing nil lines\":\"Nil lines hidden\"}</button>`;\n\n  const cols=expColumns();\n  if(!cols.length){\n    document.getElementById('tblExp').innerHTML=\n      '<p class=\"cap\">Nothing is posted in that year, so there is nothing to analyse.</p>';\n    notes(\"expNotes\",\"exp\"); return;}\n\n  const ids=[...SD,...ADM,...FIN];\n  const revs=cols.map(c=>c.budget?BUsum(REV,c.range?c.range[0]:0,c.range?c.range[1]:0)\n                                 :expOver(REV.reduce((t,id)=>t.map((v,i)=>v+PL(id)[i]),M.map(()=>0)),c));\n  /* Months across also carries the same months a year earlier and the change\n     between them, because a year of months on its own says nothing about\n     whether the year is worse than the last one. */\n  const pySpan=months?(function(){\n    const a=cols[0].range[0], b=cols[cols.length-1].range[1];\n    const pa=idx(String(+M[a].slice(0,4)-1)+M[a].slice(4)), pb=idx(String(+M[b].slice(0,4)-1)+M[b].slice(4));\n    return (pa>=0&&pb>=0)?[pa,pb]:null;})():null;\n\n  const series=id=>PL(id);\n  const rowFor=(name,arr,kind,key,depth)=>{\n    const vs=cols.map(c=>expOver(arr,c));\n    const tot=vs.reduce((t,v)=>t+(v||0),0);\n    const py=pySpan?arr.slice(pySpan[0],pySpan[1]+1).reduce((a,b)=>a+b,0):null;\n    return {name:name,vs:vs,tot:tot,py:py,kind:kind,key:key,depth:depth||0};};\n\n  let rows=ids.map(id=>rowFor(LI[id].name,series(id),\"line\",id,0));\n  if(!EXPNIL)rows=rows.filter(r=>r.vs.some(v=>v!==null&&Math.abs(v)>0.005)||Math.abs(r.py||0)>0.005);\n  /* §8c — sort by any column. The name sorts alphabetically; a figure sorts by\n     size, which is what anybody actually wants from a column of money. */\n  rows.sort(function(a,b){\n    if(EXPSORT.k===\"name\")return EXPSORT.dir*a.name.localeCompare(b.name);\n    /* dir -1 is largest first, which is the default and what anybody wants\n       from a column of money, so the subtraction runs a - b. */\n    if(EXPSORT.k===\"tot\")return EXPSORT.dir*(a.tot-b.tot);\n    if(EXPSORT.k===\"py\")return EXPSORT.dir*((a.py||0)-(b.py||0));\n    return EXPSORT.dir*((a.vs[EXPSORT.k]||0)-(b.vs[EXPSORT.k]||0));});\n\n  const money=v=>v===null?\"—\":eur(v);\n  const pcOf=(v,k)=>{const r=revs[k];\n    return (v===null||r===null||Math.abs(r)<1)?\"—\":(v/r*100).toFixed(1)+\"%\";};\n  const th=(k,label,cls)=>`<th class=\"num sortable${EXPSORT.k===k?\" sorted\":\"\"}\" data-sort=\"${k}\">${label}</th>`;\n\n  let h=`<table><thead><tr><th class=\"sortable${EXPSORT.k===\"name\"?\" sorted\":\"\"}\" data-sort=\"name\">Line</th>`;\n  cols.forEach(function(c,k){\n    h+=th(k,c.label)+`<th class=\"num pcol\">%</th>`;\n    if(!months&&k>0)h+=`<th class=\"num\">${lblShort(cols[k-1].label)} vs ${lblShort(c.label)}</th>`;\n  });\n  if(months){\n    h+=th(\"tot\",\"Year\")+`<th class=\"num pcol\">%</th>`;\n    h+=th(\"py\",\"A year earlier\")+`<th class=\"num\">Change</th>`;\n  }\n  h+=`</tr></thead><tbody>`;\n  const wide=1+cols.length*2+(months?4:(cols.length-1));\n\n  const cells=(r)=>{\n    let out=\"\";\n    r.vs.forEach(function(v,k){\n      out+=`<td class=\"num\">${money(v)}</td><td class=\"num pcol\">${pcOf(v,k)}</td>`;\n      if(!months&&k>0){const l=r.vs[k-1], d=(l===null||v===null)?null:l-v;\n        out+=`<td class=\"num\">${money(d)}</td>`;}\n    });\n    if(months){\n      out+=`<td class=\"num\"><b>${money(r.tot)}</b></td><td class=\"num pcol\">${\n        revs.reduce((t,x)=>t+(x||0),0)?((r.tot/revs.reduce((t,x)=>t+(x||0),0))*100).toFixed(1)+\"%\":\"—\"}</td>`;\n      out+=`<td class=\"num\">${r.py===null?\"—\":eur(r.py)}</td>`\n        +`<td class=\"num\">${r.py===null?\"—\":eur(r.tot-r.py)}</td>`;\n    }\n    return out;};\n\n  rows.forEach(function(r){\n    const open=EXPOPEN.line===r.key;\n    h+=`<tr class=\"expline${open?\" on\":\"\"}\" data-line=\"${r.key}\">`\n      +`<td><span class=\"twist\">${open?\"▾\":\"▸\"}</span>${r.name}</td>`+cells(r)+`</tr>`;\n    if(!open)return;\n    /* Level two: the nominal accounts under that line, across the same columns. */\n    const accs=(D.accounts||[]).filter(a=>a.line===r.key);\n    if(!accs.length){\n      h+=`<tr class=\"expsub\"><td colspan=\"${wide}\">No account is mapped to this line.</td></tr>`;\n      return;}\n    accs.map(a=>({a:a,row:rowFor(a.code+\"  \"+a.name,a.m,\"acct\",a.code,1)}))\n      .filter(x=>EXPNIL||x.row.vs.some(v=>v!==null&&Math.abs(v)>0.005)||Math.abs(x.row.py||0)>0.005)\n      .sort((x,y)=>y.row.tot-x.row.tot)\n      .forEach(function(x){\n        const on=EXPOPEN.acct===x.a.code;\n        h+=`<tr class=\"expacct${on?\" on\":\"\"}\" data-acct=\"${x.a.code}\">`\n          +`<td><span class=\"twist\">${on?\"▾\":\"▸\"}</span>${x.row.name}</td>`+cells(x.row)+`</tr>`;\n        if(!on)return;\n        /* Level three: the postings themselves, so the item can be found. */\n        /* Every column, not the first and the last: with years across the\n           last column is the OLDER one, and \"what was in it last year that is\n           not in it this year\" is the question this list exists to answer. */\n        let a0=NM-1,b0=0;\n        cols.forEach(function(c){ if(!c.range)return;\n          a0=Math.min(a0,c.range[0]); b0=Math.max(b0,c.range[1]); });\n        if(a0>b0){a0=0;b0=NM-1;}\n        const ps=pInRange(a0,b0)\n          .filter(p=>p.acc===x.a.code&&Math.abs(p.v)>0.005)\n          .sort((p,q)=>Math.abs(q.v)-Math.abs(p.v));\n        h+=`<tr class=\"expsub\"><td colspan=\"${wide}\">`\n          +(ps.length\n            ? `<div class=\"tw\"><table><thead><tr><th>Date</th><th>Journal</th><th>Reference</th>`\n              +`<th>Narrative</th><th class=\"num\">Amount</th></tr></thead><tbody>`\n              +ps.slice(0,200).map(p=>`<tr><td class=\"mono\">${p.date}</td><td class=\"mono\">${p.j}</td>`\n                +`<td class=\"mono\">${p.ref}</td><td>${p.det}</td><td class=\"num\">${eur(p.v)}</td></tr>`).join(\"\")\n              +`</tbody></table></div>`\n              +(ps.length>200?`<p class=\"cap\">The largest 200 of ${ps.length}.</p>`:\"\")\n            : `<p class=\"cap\">Nothing is posted to this account in the period shown.</p>`)\n          +`</td></tr>`;});\n  });\n  h+=\"</tbody></table>\";\n  const host=document.getElementById('tblExp');\n  host.innerHTML=h;\n\n  host.querySelectorAll(\".expline\").forEach(function(tr){\n    tr.addEventListener(\"click\",function(e){\n      if(e.target.closest(\".expsub\"))return;\n      const k=tr.dataset.line;\n      EXPOPEN=(EXPOPEN.line===k)?{line:null,acct:null}:{line:k,acct:null};\n      renderExp();});});\n  host.querySelectorAll(\".expacct\").forEach(function(tr){\n    tr.addEventListener(\"click\",function(e){\n      e.stopPropagation();\n      const k=tr.dataset.acct;\n      EXPOPEN={line:EXPOPEN.line,acct:EXPOPEN.acct===k?null:k};\n      renderExp();});});\n  host.querySelectorAll(\"[data-sort]\").forEach(function(thEl){\n    thEl.addEventListener(\"click\",function(){\n      const k=thEl.dataset.sort, kk=(k===\"name\"||k===\"tot\"||k===\"py\")?k:+k;\n      if(EXPSORT.k===kk)EXPSORT.dir=-EXPSORT.dir;\n      else EXPSORT={k:kk,dir:kk===\"name\"?1:-1};\n      renderExp();});});\n  notes(\"expNotes\",\"exp\");\n}\n" + script.slice(b);

  script += "\n/* The view switch, the nil-line switch and the year, all redrawing it. */\ndocument.addEventListener(\"click\",function(e){\n  const v=e.target.closest?e.target.closest(\"[data-view]\"):null;\n  if(v){ EXPVIEW=v.dataset.view; EXPOPEN={line:null,acct:null};\n         EXPSORT={k:EXPVIEW===\"months\"?\"tot\":0,dir:-1}; renderExp(); return; }\n  if(e.target&&e.target.id===\"expNil\"){ EXPNIL=!EXPNIL; renderExp(); }\n});\n{ const ey=document.getElementById(\"expYear\");\n  if(ey)ey.addEventListener(\"change\",function(){EXPOPEN={line:null,acct:null};renderExp();}); }\n";
}

// ---- patch 29: sales analysis, more on the top row and the ratios (§9) ----
//
// Additions only: the partner likes this screen. Five tiles, a ratios block
// under it, and one correction — custSales decided which accounts were the
// sales ledger by testing /^221/ on the code, which is A&F's chart and
// nobody else's. It uses the client's own mapping now, with the prefix as a
// fallback for a client that has none. On A&F the two agree exactly: the one
// account on Trade debtors outside 221 is Bills Receivable, and it has no
// sales journal line against it.
{
  for (const [what, was, now] of [
    ['the sales ledger test in custSales', "    if(!/^221/.test(p.acc))return;", "    if(!isSalesAcc(p.acc))return;"],
    ['the sales tiles', "    [\"Customers invoiced\",C.list.filter(x=>x.n).length,\"accounts with a sale in the period\"]]", "    [\"Customers invoiced\",C.list.filter(x=>x.n).length,\"accounts with a sale in the period\"],\n    /* FIX-3 §9a — five more, for the conversation the six above start. */\n    (function(){var f=custFirst().f,n=0;\n      C.list.forEach(function(x){if(x.v<=0)return;var d=f[x.code];\n        if(d&&d>=M[a]+\"-01\"&&d<=M[b]+\"-31\")n++;});\n      return [\"New customers\",n,\"their first sale ever fell in this period\"];})(),\n    (function(){if(!CP)return [\"Customers lost\",\"\\u2014\",\"no comparative loaded\"];\n      var now={};C.list.forEach(function(x){if(x.v>0)now[x.code]=1;});\n      var n=CP.list.filter(function(x){return x.v>0&&!now[x.code]}).length;\n      return [\"Customers lost\",n,\"bought in the same period last year, nothing in this one\"];})(),\n    (function(){var sold=C.list.filter(x=>x.v>0).sort((x,y)=>y.v-x.v);\n      /* Net of credit notes on both sides of the division: C.gross is\n         invoices alone, and a share that mixes the two understates it. */\n      var all=sold.reduce((s,x)=>s+x.v,0), t=sold.slice(0,10).reduce((s,x)=>s+x.v,0);\n      return [\"Top ten as a share\",all?(t/all*100).toFixed(1)+\"%\":\"\\u2014\",\n              \"the ten largest, of \"+eur(all)+\" net of credit notes\"];})(),\n    [\"Credit notes as a share\",C.gross?(C.ret/C.gross*100).toFixed(1)+\"%\":\"\\u2014\",\n     C.rv+\" note\"+(C.rv===1?\"\":\"s\")+\", \"+eur(C.ret)+\" returned\"],\n    (function(){var n=b-a+1;\n      return [\"Sales per month\",eur(nv/n),\"run rate across the \"+n+\" month\"+(n===1?\"\":\"s\")+\" shown\"];})()]"],
    ['the end of renderSales', "  notes(\"salesNotes\",\"sales\");", "  renderSalesRatios();\n  notes(\"salesNotes\",\"sales\");"],
  ]) {
    if (!script.includes(was)) throw new Error(what + ' is not where it was.');
    script = script.replace(was, now);
  }
  script += "\n/* ---------- sales analysis: the additions (FIX-3 §9) ---------- */\n/*\n * The partner likes this screen, so nothing here rebuilds it. Five more tiles\n * on the top row, and a block of sales ratios underneath — about the customers\n * rather than the position, which is why they are not on the Ratios screen.\n *\n * Same three rules as §6: every ratio shows the two figures it is made from,\n * every ratio runs across the same columns, and one that cannot be worked out\n * says why.\n */\n\n/* Which accounts ARE the sales ledger.\n *\n * custSales tested /^221/ on the account code, which is A&F's chart and nobody\n * else's: a client whose sales ledger starts anywhere else would have had an\n * empty Sales analysis and no indication why. This takes the accounts the\n * client's own mapping puts on Trade debtors, and keeps the prefix only as a\n * fallback for a client with no mapping yet. On A&F the two agree — the one\n * B-120 account outside 221 is Bills Receivable, which has no sales journal\n * line against it — so nothing the partner has looked at changes.\n */\nlet SLACC=null;\nfunction salesAccounts(){\n  if(SLACC&&SLACC.cid===CID)return SLACC;\n  const set=new Set((D.accounts||[]).filter(a=>a.line===\"B-120\").map(a=>a.code));\n  SLACC={cid:CID,set:set,ok:set.size>0};\n  return SLACC;\n}\nfunction isSalesAcc(code){\n  const S=salesAccounts();\n  return S.ok?S.set.has(code):/^221/.test(code);\n}\n\n/* The first invoice a customer was ever sent, for \"new customers\". Read once\n   over the whole ledger, because \"first ever\" means exactly that. */\nlet SLFIRST=null;\nfunction custFirst(){\n  if(SLFIRST&&SLFIRST.cid===CID)return SLFIRST;\n  const f={};\n  postings().rows.forEach(function(p){\n    if(!SALESJ.includes(p.j))return;\n    if(!isSalesAcc(p.acc))return;\n    if(p.v<=0)return;                       /* an invoice, not a credit note */\n    if(!f[p.acc]||p.date<f[p.acc])f[p.acc]=p.date;});\n  SLFIRST={cid:CID,f:f};\n  return SLFIRST;\n}\n\n/* Everything about one column of the sales ratios. `prior` is the column one\n   older, which the growth ratios want. */\nfunction salesFigures(col){\n  if(col.missing||col.budget)return null;\n  const a=col.range[0], b=col.range[1];\n  const C=custSales(a,b);\n  const sold=C.list.filter(x=>x.v>0).sort((x,y)=>y.v-x.v);\n  /* One base for every share: what the customers came to, net of their own\n     credit notes. C.gross is invoices ALONE, and a share of sales that divides\n     a figure net of notes by a figure gross of them is wrong in the direction\n     nobody would notice. */\n  const soldTotal=sold.reduce((t,x)=>t+x.v,0);\n  const share=(n)=>soldTotal?sold.slice(0,n).reduce((t,x)=>t+x.v,0):null;\n  const first=custFirst().f;\n  let fresh=0, repeat=0, freshN=0;\n  sold.forEach(function(x){\n    const d=first[x.code];\n    if(d&&d>=M[a]+\"-01\"&&d<=M[b]+\"-31\"){fresh+=x.v;freshN++;}\n    else repeat+=x.v;});\n  const net=REV.reduce((t,id)=>t+PL(id).slice(a,b+1).reduce((q,w)=>q+w,0),0);\n  const cos=COS.reduce((t,id)=>t+PL(id).slice(a,b+1).reduce((q,w)=>q+w,0),0);\n  return {C:C, sold:sold, top:share, soldTotal:soldTotal, gross:C.gross, ret:C.ret, inv:C.inv, rv:C.rv,\n          n:sold.length, fresh:fresh, freshN:freshN, repeat:repeat,\n          net:net, cos:cos, gp:net-cos,\n          deb:BS(\"B-120\")[b], days:daysIn(a,b), a:a, b:b};\n}\n\n/* Each column's sales for a given set of customer codes — for the growth of the\n   ten largest against everyone else. */\nfunction soldTo(F,codes){\n  return F.sold.filter(x=>codes.has(x.code)).reduce((t,x)=>t+x.v,0);\n}\n\nconst SLRATIOS=[\n [\"Concentration\",[\n  {n:\"The largest customer, as a share\",u:\"pc\",p:[\"That customer\",\"Invoiced, net of credit notes\"],\n   f:F=>[F.top(1),F.soldTotal],why:F=>F.n?null:\"nothing was invoiced in the period\"},\n  {n:\"The five largest, as a share\",u:\"pc\",p:[\"Those five\",\"Invoiced, net of credit notes\"],\n   f:F=>[F.top(5),F.soldTotal],why:F=>F.n?null:\"nothing was invoiced in the period\"},\n  {n:\"The ten largest, as a share\",u:\"pc\",p:[\"Those ten\",\"Invoiced, net of credit notes\"],\n   f:F=>[F.top(10),F.soldTotal],why:F=>F.n?null:\"nothing was invoiced in the period\"}]],\n [\"Mix\",[\n  {n:\"New customer revenue, as a share\",u:\"pc\",p:[\"Customers whose first sale ever fell here\",\"Invoiced, net of credit notes\"],\n   f:F=>[F.fresh,F.soldTotal],why:F=>F.n?null:\"nothing was invoiced in the period\"},\n  {n:\"Revenue per customer\",u:\"eur\",p:[\"Invoiced, net of credit notes\",\"Customers invoiced\"],\n   f:F=>[F.soldTotal,F.n],why:F=>F.n?null:\"no customer was invoiced\"}]],\n [\"Quality\",[\n  {n:\"Credit note rate\",u:\"pc\",p:[\"Credit notes\",\"Invoiced in the period\"],\n   f:F=>[F.ret,F.gross],why:F=>F.gross?null:\"nothing was invoiced in the period\"},\n  {n:\"Average invoice value\",u:\"eur\",p:[\"Invoiced in the period\",\"Invoices issued\"],\n   f:F=>[F.gross,F.inv],why:F=>F.inv?null:\"no invoice was issued\"},\n  {n:\"Gross margin\",u:\"pc\",p:[\"Gross profit\",\"Net sales\"],\n   f:F=>[F.gp,F.net],why:F=>Math.abs(F.net)<0.005?\"no sales in the period\":null}]],\n [\"Collection\",[\n  {n:\"Debtor days\",u:\"days\",p:[\"Trade debtors at the period end\",\"Net sales for the period\"],\n   f:F=>[F.deb,F.net],v:F=>F.deb/F.net*F.days,\n   why:F=>Math.abs(F.net)<0.005?\"no sales in the period\":null},\n  {n:\"Debtors over 90 days, as a share\",u:\"pc\",p:[\"Over 90 days\",\"Owed to the business\"],\n   f:F=>[(D.agetot||{}).deb?D.agetot.deb[3]:undefined,(D.agetot||{}).debpos],\n   asAt:true,\n   why:F=>!(D.agetot&&D.agetot.deb)?\"no ageing has been loaded\":null}]],\n [\"Growth\",[\n  {n:\"Revenue growth\",u:\"pc\",p:[\"Net sales this column\",\"Net sales the column before\"],\n   needsPrior:true,f:(F,P)=>[F.net,P.net],\n   why:(F,P)=>Math.abs(P.net)<0.005?\"nothing to grow from\":null},\n  {n:\"Growth of the ten largest\",u:\"pc\",p:[\"Those ten, this column\",\"The same ten, the column before\"],\n   needsPrior:true,\n   f:function(F,P){const c=new Set(F.sold.slice(0,10).map(x=>x.code));\n     return [soldTo(F,c),soldTo(P,c)];},\n   why:function(F,P){const c=new Set(F.sold.slice(0,10).map(x=>x.code));\n     return Math.abs(soldTo(P,c))<0.005?\"they bought nothing in the column before\":null;}},\n  {n:\"Growth of everyone else\",u:\"pc\",p:[\"Everyone else, this column\",\"The same, the column before\"],\n   needsPrior:true,\n   f:function(F,P){const c=new Set(F.sold.slice(0,10).map(x=>x.code));\n     return [F.soldTotal-soldTo(F,c),P.soldTotal-soldTo(P,c)];},\n   why:function(F,P){const c=new Set(F.sold.slice(0,10).map(x=>x.code));\n     return Math.abs(P.soldTotal-soldTo(P,c))<0.005?\"they bought nothing in the column before\":null;}}]]\n];\n\nfunction renderSalesRatios(){\n  const host=document.getElementById('tblSlRatios');\n  if(!host)return;\n  cmpCtl(\"slr\",\"slrCmpBar\",renderSalesRatios);\n  const r=periodRange(\"sl\");\n  const cols=[{label:spanLbl(r),range:r}].concat(cmpCols(\"slr\").map(function(c){\n    return c.missing?{label:c.label,missing:c.missing}\n                    :(c.budget?{label:c.label,missing:\"a sales ratio is not budgeted\"}\n                              :{label:c.label,range:c.range});}));\n  const F=cols.map(salesFigures);\n\n  let h=`<table><thead><tr><th>Ratio</th>`\n    +cols.map(c=>`<th class=\"num\">${c.label}</th>`).join(\"\")+`</tr></thead><tbody>`;\n  SLRATIOS.forEach(function(g){\n    h+=`<tr class=\"sec\"><td colspan=\"${cols.length+1}\">${g[0]}</td></tr>`;\n    g[1].forEach(function(rt){\n      const cells=[],parts=[[],[]];\n      cols.forEach(function(c,k){\n        const f=F[k], p=F[k+1];\n        const none=()=>{parts[0].push('<td class=\"num\">—</td>');parts[1].push('<td class=\"num\">—</td>');};\n        if(!f){cells.push(`<td class=\"num rwhy\">${c.missing}</td>`);none();return;}\n        if(rt.needsPrior&&!p){\n          cells.push('<td class=\"num rwhy\">no earlier column to compare with</td>');none();return;}\n        /* The ageing is held as at the latest month and nowhere else, so a\n           ratio drawn from it can only answer for the column that ends there. */\n        if(rt.asAt&&k>0){\n          cells.push('<td class=\"num rwhy\">the ageing is held only as at the latest month</td>');none();return;}\n        const why=rt.why?rt.why(f,p):null;\n        const pr=rt.f(f,p);\n        parts[0].push(`<td class=\"num\">${pr[0]===undefined||pr[0]===null?\"—\":eur(pr[0])}</td>`);\n        parts[1].push(`<td class=\"num\">${pr[1]===undefined||pr[1]===null?\"—\":eur(pr[1])}</td>`);\n        cells.push(why?`<td class=\"num rwhy\">${why}</td>`\n                      :`<td class=\"num\"><b>${ratioShow(rt.u,ratioValue(rt,f,p))}</b></td>`);\n      });\n      h+=`<tr><td>${rt.n}</td>${cells.join(\"\")}</tr>`;\n      h+=`<tr class=\"rpart\"><td>${rt.p[0]}</td>${parts[0].join(\"\")}</tr>`;\n      h+=`<tr class=\"rpart\"><td>${rt.p[1]}</td>${parts[1].join(\"\")}</tr>`;\n    });\n  });\n  h+=\"</tbody></table>\";\n  host.innerHTML=h;\n\n  /* \"Average days to pay, largest customers first\". The ledger holds no\n     matching of receipts to invoices, so this is what it can honestly answer:\n     each customer's own balance measured against what they buy. */\n  const f0=F[0], el=document.getElementById('tblSlPay');\n  if(el){\n    if(!f0){el.innerHTML='<p class=\"cap\">Nothing to measure in this period.</p>';}\n    else{\n      const bal={};(D.deb||[]).forEach(x=>bal[x.code]=x.bal);\n      const rows=f0.sold.slice(0,15).map(function(x){\n        const b=bal[x.code], d=(b===undefined||Math.abs(x.v)<0.005)?null:b/x.v*f0.days;\n        return {x:x,b:b,d:d};});\n      el.innerHTML=`<table><thead><tr><th>Customer</th><th class=\"num\">Invoiced in the period</th>`\n        +`<th class=\"num\">Owed now</th><th class=\"num\">Days</th></tr></thead><tbody>`\n        +rows.map(function(q){\n          return `<tr><td>${q.x.name}</td><td class=\"num\">${eur(q.x.v)}</td>`\n            +`<td class=\"num\">${q.b===undefined?'<span class=\"rwhy\">no ageing</span>':eur(q.b)}</td>`\n            +`<td class=\"num\">${q.d===null?'<span class=\"rwhy\">—</span>':Math.round(q.d)+\" days\"}</td></tr>`;}).join(\"\")\n        +`</tbody></table>`;\n    }\n  }\n}\n";
}
...
/* FIX-3 §8 — a line that opens looks like one, and what it opens to is
   plainly underneath it rather than beside it. */
.expline,.expacct{cursor:pointer}
.expline:hover>td,.expacct:hover>td{background:var(--grid)}
.expline.on>td,.expacct.on>td{background:var(--grid);font-weight:600}
.expacct>td:first-child{padding-left:26px;color:var(--ink-2)}
.expsub>td{padding:6px 10px 14px 26px;background:var(--surface)}
...
{
  // FIX-3 §8a — months across or years across, chosen at the top, with a year
  // picker that only the months view needs.
  const ec2 = '<div class="controls" id="expCtl"></div>';
  if (!assembled.includes(ec2)) throw new Error('the Expense analysis controls are not where they were.');
  assembled = assembled.replace(ec2,
    '<div class="controls perbtns" id="expView"></div>'
    + '<div class="controls" id="expYearBar">'
      + '<label class="ctl" for="expYear">Year</label><select id="expYear"></select>'
      + '</div>'
    + ec2);
}

{
  // FIX-3 §9b — sales ratios, under the screen rather than in it: the partner
  // likes this screen and asked for additions, not a rebuild.
  const sn = '<div class="note" id="slNote"></div>';
  if (!assembled.includes(sn)) throw new Error('the sales note is not where it was.');
  assembled = assembled.replace(sn, [
    '<h3 style="margin:22px 0 4px">Sales ratios</h3>',
    '<p class="sub">About the customers rather than the position — the ratios of the '
      + 'balance sheet are on the Ratios screen. Every ratio shows the two figures it '
      + 'is made from, and one that cannot be worked out says why.</p>',
    '<div class="chartbar cmpbar" id="slrCmpBar"></div>',
    '<div class="tw" id="tblSlRatios"></div>',
    '<h3 style="margin:22px 0 4px">Average days to pay, largest customers first</h3>',
    '<p class="sub">The ledger holds no matching of receipts to invoices, so this is '
      + 'what it can honestly answer: each customer\'s own balance measured against '
      + 'what they buy. The balance is as at the latest month held.</p>',
