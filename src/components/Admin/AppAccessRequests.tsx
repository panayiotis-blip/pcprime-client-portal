...
type Req = { id: number; app_key: string; client_name: string; full_name: string | null; email: string | null; phone: string | null; message: string | null; created_at: string };
...
export default function AppAccessRequests() {
  ...
  const approve = async (r: Req) => {
    ...
    setBusy(r.id);
    try {
      const res = await api.approveAppRequest(r.id, Number(s.clientId), r.app_key, s.role || 'editor');
      alert(res.invited
        ? `Approved — an invite email was sent to ${r.email} to set a password.`
        : `Approved — ${r.email} already had an account, so their app access is active now.`);
      load();
    } catch (e: any) { alert(e?.message || 'Failed'); } finally { setBusy(null); }
  };
  const reject = async (r: Req) => {
    if (!confirm(`Reject the request from ${r.email || r.client_name} (${r.client_name})?`)) return;
    setBusy(r.id);
  ...
  return (
    <div className="dashboard" style={{ padding: '1rem 1.5rem' }}>
      ...
      <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 14px' }}>
        People who requested app access. Approving grants the app to their <strong>email</strong> — an existing account is reused,
        a new one is invited to set a password — so pick which client it belongs to and a role.
      </p>
          ...
          {reqs.map(r => (
            <div key={r.id} style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: 14, background: '#fff' }}>
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                ...
                <div><div style={{ fontSize: 11, color: '#94a3b8' }}>Email</div><strong>{r.email || '—'}</strong></div>
                <div><div style={{ fontSize: 11, color: '#94a3b8' }}>App</div>{r.app_key}</div>
                <div><div style={{ fontSize: 11, color: '#94a3b8' }}>Phone</div>{r.phone || '—'}</div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', marginTop: 12, flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-primary" disabled={busy === r.id} onClick={() => approve(r)}>Approve → invite &amp; grant</button>
