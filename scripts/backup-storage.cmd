@echo off
rem ============================================================
rem  Nightly wrapper for scripts/backup-storage.mjs
rem ============================================================
...
rem  backups\logs\backup-YYYY-MM-DD.log and passes the exit code back, so the
rem  task's Last Run Result is the truth and the log says why.
rem
rem  Also runnable by hand — double-click it, or:  scripts\backup-storage.cmd
rem ============================================================
setlocal
...
rem log files called things like backup-18/08/2026.log, which cannot exist.
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd"') do set "TODAY=%%i"
set "LOG=%LOGDIR%\backup-%TODAY%.log"
...
powershell -NoProfile -Command "Get-ChildItem '%LOGDIR%\backup-*.log' -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -lt (Get-Date).AddDays(-30) } | Remove-Item -Force -ErrorAction SilentlyContinue" >nul 2>&1
